...
import dataclasses, logging, queue, shutil, struct, threading
...
FONT_MONO=("Consolas",9); FONT_UI=("Segoe UI",10)
FONT_UI_B=("Segoe UI",10,"bold"); FONT_TITLE=("Segoe UI",13,"bold")
...
class FiasConfig:
    ...
    @property
    def log_file(self) -> Path: return self.base_dir/"script.log"
    @property
    def last_tuesday(self) -> str:
        t = datetime.now(); d = (t.weekday()-1)%7
...
def setup_logger(config: FiasConfig, log_queue: queue.Queue) -> logging.Logger:
    ...
    fh.setFormatter(fmt)
    qh = QueueHandler(log_queue); qh.setFormatter(fmt)
    lg = logging.getLogger(f"fias_{id(log_queue)}")
    lg.setLevel(logging.DEBUG); lg.addHandler(fh); lg.addHandler(qh)
    return lg
...
_MERGE_GAP = 1*1024*1024 # Сливать соседние записи если расстояние < 1 МБ

@dataclass
class ZipEntry:
    filename: str; filename_bytes: bytes
    flags: int; compress_method: int
    mod_time: int; mod_date: int
    crc32: int; compressed_size: int; uncompressed_size: int
    local_header_offset: int
    version_made: int; version_needed: int; comment_bytes: bytes

def _rget(url: str, s: int, e: int, timeout: int=120) -> bytes:
...
def _head(url: str) -> tuple[bool,int]:
    ...
    with urllib.request.urlopen(req, timeout=30) as r:
        return r.headers.get("Accept-Ranges","").lower()=="bytes", int(r.headers.get("Content-Length",0))

def _find_eocd(data: bytes) -> int:
    for i in range(len(data)-22,-1,-1):
        if data[i:i+4]==_EOCD: return i
    raise RuntimeError("EOCD not found — archive may be corrupted")
...
def _parse_eocd(url: str, fsize: int) -> tuple[int,int,int]:
    ...
    if z64p>=0 and tail[z64p:z64p+4]==_Z64L:
        ...
        _,_,_,_,_,_,entries,_,cd_size,cd_off = struct.unpack_from("<4sQHHIIQQQQ",z64)
    ...
    while pos+46<=len(data):
        ...
        if cs==0xFFFFFFFF or us==0xFFFFFFFF or lho==0xFFFFFFFF:
            ...
            while ep2+4<=len(ex):
                ...
                if eid==0x0001:
                    ef=ex[ep2+4:ep2+4+esz]; fp=0
                    if us==0xFFFFFFFF and fp+8<=len(ef): us=struct.unpack_from("<Q",ef,fp)[0]; fp+=8
                    if cs==0xFFFFFFFF and fp+8<=len(ef): cs=struct.unpack_from("<Q",ef,fp)[0]; fp+=8
                    if lho==0xFFFFFFFF and fp+8<=len(ef): lho=struct.unpack_from("<Q",ef,fp)[0]
                    break
                ep2+=4+esz
        try: name=fn.decode("utf-8")
        except: name=fn.decode("cp437",errors="replace")
        out.append(ZipEntry(name,fn,fl,cm,mt,md,crc,cs,us,lho,vm,vn,co))
        pos+=46+fnl+exl+cml
    return out

def _is_target(e: ZipEntry, regions: tuple[str,...]) -> bool:
    if e.compressed_size==0: return False
...
def _batches(entries: list[ZipEntry]) -> list[list[ZipEntry]]:
    ...
    for e in se[1:]:
        last=bs[-1][-1]
        end=last.local_header_offset+30+len(last.filename_bytes)+last.compressed_size+512
        if e.local_header_offset-end<=_MERGE_GAP: bs[-1].append(e)
        else: bs.append([e])
    return bs
...
class _ZipWriter:
    ...
    def __init__(self,path:Path):
        self._f=open(path,"wb"); self._cd=[]; self._pos=0
    def add(self,e:ZipEntry,raw:bytes):
        fn=e.filename_bytes
        lh=struct.pack("<4sHHHHHIIIHH",_LFH,e.version_needed,
                       e.flags&~8,e.compress_method,e.mod_time,e.mod_date,
                       e.crc32,len(raw),e.uncompressed_size,len(fn),0)
        off=self._pos; blk=lh+fn+raw
        self._f.write(blk); self._pos+=len(blk)
        cdh=struct.pack("<4sHHHHHHIIIHHHHHII",_CDH,e.version_made,e.version_needed,
                        e.flags&~8,e.compress_method,e.mod_time,e.mod_date,
                        e.crc32,len(raw),e.uncompressed_size,
                        len(fn),0,len(e.comment_bytes),0,0,0,off)
        self._cd.append(cdh+fn+e.comment_bytes)
    def close(self):
        cds=self._pos; cd=b"".join(self._cd); self._f.write(cd)
        self._f.write(struct.pack("<4sHHHHIIH",_EOCD,0,0,
                                  len(self._cd),len(self._cd),len(cd),cds,0))
        self._f.close()
    def __enter__(self): return self
    def __exit__(self,et,*_):
        if et is None: self.close()
        else: self._f.close()

def smart_download(config:FiasConfig, logger:logging.Logger,
                   on_progress:Callable[[float,float,float],None],
                   on_status:Callable[[str],None],
                   stop:threading.Event) -> None:
    url=config.download_url; regions=config.region_folders
    config.base_dir.mkdir(parents=True,exist_ok=True)

    # 1. HEAD
    on_status("Проверка сервера..."); logger.info("Архив: %s", url)
    try: range_ok,fsize = _head(url)
    except Exception as ex: raise RuntimeError(f"Сервер недоступен: {ex}") from ex
    ...
    if not range_ok or fsize==0:
        ...
        _fallback(config,logger,on_progress,on_status,stop); return
    ...
    try: cd_off,cd_sz,n_entries = _parse_eocd(url,fsize)
    except Exception as ex: raise RuntimeError(f"Ошибка чтения ZIP: {ex}") from ex
    ...
    except Exception as ex: raise RuntimeError(f"Ошибка CD: {ex}") from ex
    ...
    nb=sum(e.compressed_size for e in selected); nm=nb/1024/1024
    saving=(1-nb/max(fsize,1))*100
    ...
    logger.info("Объём: %.1f МБ  (экономия %.0f%% от %.0f МБ)", nm, saving, total_mb)
...
def _fallback(config,logger,on_progress,on_status,stop):
    ...
    def hook(bn,bs2,ts):
        ...
        if stop.is_set(): raise InterruptedError()
        if ts<=0: return
        done=min(bn*bs2,ts); pt=int(done*1000/ts)
        if pt>lpt: lpt=pt; on_progress(pt/10,done/1024/1024,ts/1024/1024)
    try: urllib.request.urlretrieve(config.download_url,str(config.raw_archive_path),hook)
    except urllib.error.HTTPError as ex: raise RuntimeError(f"HTTP {ex.code}") from ex
    except urllib.error.URLError as ex: raise RuntimeError(f"Сеть: {ex.reason}") from ex
    ...
    tmp=config.download_dir/"filtered"; tmp.mkdir(parents=True,exist_ok=True)
    try:
        with zipfile.ZipFile(config.raw_archive_path,"r") as src:
            ms=[m for m in src.infolist() if not m.is_dir()
                and _is_target(ZipEntry(m.filename,m.filename.encode(),0,0,0,0,
                                        m.CRC,m.compress_size,m.file_size,0,0,0,b""),
                               config.region_folders)]
            for m in ms: src.extract(m,tmp)
        with zipfile.ZipFile(config.filtered_archive_path,"w",zipfile.ZIP_DEFLATED) as dst:
            for f in tmp.rglob("*"):
                if f.is_file(): dst.write(f,f.relative_to(tmp))
    finally:
        shutil.rmtree(tmp,ignore_errors=True)
        config.raw_archive_path.unlink(missing_ok=True)
        try: config.download_dir.rmdir()
        except: pass
...
def purge_old(config:FiasConfig, logger:logging.Logger) -> None:
    ...
    Удаляет только файлы, созданные этим скриптом:
      ...
      - Временный исходный архив gar_xml.zip если он остался
    ...
    for f in config.base_dir.glob("gar_xml_*.zip"):
        if f.name == config.filtered_archive_name:
            continue  # это актуальный архив текущей недели — пропускаем
    ...
    #    Скрипт создаёт их для хранения raw-архива во время обработки.
    import re
    ...
    for subdir in sorted(config.base_dir.iterdir(), reverse=True):
        ...
        if not date_pattern.match(subdir.name):
            continue  # папка с нестандартным именем — не трогаем
        # Удаляем только если папка старше cutoff и содержит только наши файлы.
    ...
    logger.info("Очистка: удалено %d объектов (старше %d нед.)", d, config.weeks_to_keep)

# ── Виджеты ───────────────────────────────────────────────────────────────────
class HoverButton(tk.Label):
    def __init__(self,parent,text,command,bg=None,bg_hover=None,fg="white",disabled_bg=None,**kw):
        self._bg=bg or CLR["accent"]; self._bgh=bg_hover or CLR["btn_hover"]
        self._bgd=disabled_bg or CLR["border"]; self._cmd=command; self._on=True
        super().__init__(parent,text=text,bg=self._bg,fg=fg,font=FONT_UI_B,
    ...
    def set_enabled(self,v):
        self._on=v; self.config(bg=self._bg if v else self._bgd,cursor="hand2" if v else "arrow")

class ProgressBar(tk.Canvas):
    def __init__(self,parent,**kw):
        super().__init__(parent,bg=CLR["progress_bg"],highlightthickness=1,
                         highlightbackground=CLR["border"],height=28,**kw)
        self._p=0.0; self.bind("<Configure>",lambda _:self._draw())
    def set(self,p):
        self._p=max(0.0,min(p,100.0)); self._draw()
    def _draw(self):
        self.delete("all"); w,h=self.winfo_width(),self.winfo_height()
        if w<2: return
        fw=int(w*self._p/100); self.create_rectangle(0,0,w,h,fill=CLR["progress_bg"],outline="")
        if fw>0:
            for i in range(fw):
                t=i/max(fw-1,1)
                r=int(0x4F+(0x7C-0x4F)*t); g=int(0x8E+(0x5B-0x8E)*t); b=int(0xF7+(0xF5-0xF7)*t)
                self.create_line(i,2,i,h-2,fill=f"#{r:02x}{g:02x}{b:02x}")
        self.create_text(w//2,h//2,text=f"{self._p:.1f}%" if self._p>0 else "Ожидание...",
                         fill="white",font=FONT_UI_B)

class StatusBadge(tk.Frame):
    ST={"idle":("●",CLR["text_dim"],"Ожидание"),
    ...
    def __init__(self,parent):
        ...
        self._i.pack(side="left",padx=(0,5)); self._t.pack(side="left")
        self.set("idle")
    ...
    def set_text(self,text): self._t.config(text=text)
...
class RegionPicker(tk.Frame):
    def __init__(self,parent,**kw):
        ...
        self._build()
    def _build(self):
        h=tk.Frame(self,bg=CLR["panel"]); h.pack(fill="x",pady=(0,6))
        tk.Label(h,text="🗺  Регионы для извлечения",font=FONT_UI_B,
                 fg=CLR["text"],bg=CLR["panel"]).pack(side="left")
        self._cnt=tk.Label(h,text="выбрано: 0",font=FONT_SMALL,
                           fg=CLR["accent"],bg=CLR["panel"]); self._cnt.pack(side="right")
        sf=tk.Frame(self,bg=CLR["panel"]); sf.pack(fill="x",pady=(0,6))
        self._sv=tk.StringVar(); self._sv.trace_add("write",lambda *_:self._filter())
        tk.Entry(sf,textvariable=self._sv,font=FONT_UI,bg=CLR["log_bg"],fg=CLR["text"],
                 insertbackground=CLR["accent"],relief="flat",bd=0).pack(
                     side="left",fill="x",expand=True,ipady=5,padx=(0,8))
        tk.Label(sf,text="🔍",font=FONT_UI,fg=CLR["text_dim"],bg=CLR["panel"]).pack(side="left")
        bf=tk.Frame(self,bg=CLR["panel"]); bf.pack(fill="x",pady=(0,6))
        for lbl,v in (("Выбрать все",True),("Снять все",False)):
            HoverButton(bf,text=lbl,command=lambda x=v:self._all(x),
                        bg=CLR["border"],bg_hover=CLR["accent"],
                        fg=CLR["text_dim"]).pack(side="left",padx=(0,6))
        wr=tk.Frame(self,bg=CLR["border"],padx=1,pady=1); wr.pack(fill="both",expand=True)
        self._cv=tk.Canvas(wr,bg=CLR["panel"],highlightthickness=0,bd=0)
        sb=tk.Scrollbar(wr,orient="vertical",command=self._cv.yview,
        ...
        for code in sorted(RF_REGIONS): self._add_row(code)
        self._upd()
    def _add_row(self,code):
        nm=RF_REGIONS[code]; var=tk.BooleanVar(value=False)
        self._vars[code]=var
        row=tk.Frame(self._lf,bg=CLR["panel"],cursor="hand2"); row.pack(fill="x",padx=4,pady=1)
        self._rows[code]=row
        cc=tk.Canvas(row,width=16,height=16,bd=0,highlightthickness=0,bg=CLR["panel"])
        cc.pack(side="left",padx=(6,8),pady=4); self._ccs[code]=cc
        tk.Label(row,text=code,font=("Consolas",9,"bold"),fg=CLR["accent"],
                 bg=CLR["panel"],width=3,anchor="e").pack(side="left",padx=(0,6))
        tk.Label(row,text=nm,font=FONT_UI,fg=CLR["text"],
                 bg=CLR["panel"],anchor="w").pack(side="left",fill="x",expand=True)
        def tog(_c=code):
            self._vars[_c].set(not self._vars[_c].get()); self._dc(_c); self._upd()
        ...
        self._dc(code)
    def _dc(self,code):
        cc=self._ccs[code]; ck=self._vars[code].get()
        cc.delete("all"); col=CLR["check_on"] if ck else CLR["border"]
        cc.create_rectangle(1,1,15,15,outline=col,fill=CLR["panel"],width=2)
        if ck: cc.create_rectangle(4,4,12,12,fill=CLR["check_on"],outline="")
    def _filter(self):
        q=self._sv.get().lower().strip()
        for c,r in self._rows.items():
            vis=not q or q in c or q in RF_REGIONS[c].lower()
            if vis: r.pack(fill="x",padx=4,pady=1)
            else: r.pack_forget()
...
class FiasApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self._cfg=FiasConfig(); self._q:queue.Queue=queue.Queue()
        self._stop=threading.Event(); self._worker=None
        self._fmt=logging.Formatter("[%(asctime)s] %(levelname)-8s %(message)s","%Y-%m-%d %H:%M:%S")
        self._build_win(); self._build_ui(); self._poll()

    def _build_win(self):
        self.title("ФИАС ГАР — Range-загрузчик регионов")
        self.configure(bg=CLR["bg"]); self.resizable(True,True); self.minsize(900,640)
        self.update_idletasks(); w,h=1100,760
        self.geometry(f"{w}x{h}+{(self.winfo_screenwidth()-w)//2}+{(self.winfo_screenheight()-h)//2}")

    def _build_ui(self):
        hdr=tk.Frame(self,bg=CLR["panel"]); hdr.pack(fill="x",side="top")
        tk.Frame(hdr,bg=CLR["accent"],height=3).pack(fill="x")
        tr=tk.Frame(hdr,bg=CLR["panel"],padx=20,pady=14); tr.pack(fill="x")
        ...
        tk.Label(tr,text="Provision LLC  |  2025",font=FONT_SMALL,
                 fg=CLR["text_dim"],bg=CLR["panel"]).pack(side="right",padx=(0,20))
        ...
        ft=tk.Frame(self,bg=CLR["panel"],padx=20,pady=12); ft.pack(side="bottom",fill="x")
        self._cbtn=HoverButton(ft,text="⏹  Остановить",command=self._cancel,
                               bg=CLR["border"],bg_hover="#5C1A1A",fg=CLR["error"])
        self._cbtn.pack(side="right",padx=(8,0)); self._cbtn.set_enabled(False)
        self._sbtn=HoverButton(ft,text="▶  Начать загрузку",command=self._start)
        self._sbtn.pack(side="right")
        self._lbl=tk.Label(ft,text=f"Лог → {self._cfg.log_file}",
                           font=FONT_SMALL,fg=CLR["text_dim"],bg=CLR["panel"]); self._lbl.pack(side="left")
        # Body
        body=tk.Frame(self,bg=CLR["bg"],padx=16,pady=14); body.pack(side="top",fill="both",expand=True)
        body.columnconfigure(0,weight=3); body.columnconfigure(1,weight=2); body.rowconfigure(0,weight=1)
        left=tk.Frame(body,bg=CLR["bg"]); left.grid(row=0,column=0,sticky="nsew",padx=(0,10))
        left.rowconfigure(2,weight=1); left.columnconfigure(0,weight=1)
        self._build_path(left); self._build_prog(left); self._build_log(left)
        right=tk.Frame(body,bg=CLR["panel"],padx=14,pady=12); right.grid(row=0,column=1,sticky="nsew")
        right.rowconfigure(0,weight=1); right.columnconfigure(0,weight=1)
        self._rp=RegionPicker(right); self._rp.pack(fill="both",expand=True)

    def _build_path(self,p):
        f=tk.Frame(p,bg=CLR["panel"],padx=14,pady=12); f.grid(row=0,column=0,sticky="ew",pady=(0,10))
        tk.Label(f,text="📁  Папка сохранения",font=FONT_UI_B,fg=CLR["text"],bg=CLR["panel"]).pack(anchor="w",pady=(0,6))
        row=tk.Frame(f,bg=CLR["panel"]); row.pack(fill="x")
        self._pv=tk.StringVar(value=str(self._cfg.base_dir))
        tk.Entry(row,textvariable=self._pv,font=FONT_MONO,bg=CLR["log_bg"],fg=CLR["text"],
                 insertbackground=CLR["accent"],relief="flat",bd=0).pack(side="left",fill="x",expand=True,ipady=7,padx=(0,10))
        HoverButton(row,text="Обзор...",command=self._browse,bg=CLR["border"],bg_hover=CLR["accent"]).pack(side="left")

    def _build_prog(self,p):
        f=tk.Frame(p,bg=CLR["panel"],padx=14,pady=12); f.grid(row=1,column=0,sticky="ew",pady=(0,10))
        top=tk.Frame(f,bg=CLR["panel"]); top.pack(fill="x",pady=(0,8))
        tk.Label(top,text="⬇  Прогресс загрузки",font=FONT_UI_B,fg=CLR["text"],bg=CLR["panel"]).pack(side="left")
        self._sz=tk.Label(top,text="",font=FONT_SMALL,fg=CLR["text_dim"],bg=CLR["panel"]); self._sz.pack(side="right")
        self._pb=ProgressBar(f); self._pb.pack(fill="x")
        sr=tk.Frame(f,bg=CLR["panel"]); sr.pack(fill="x",pady=(8,0))
        self._st=StatusBadge(sr); self._st.pack(side="left")

    def _build_log(self,p):
        f=tk.Frame(p,bg=CLR["panel"],padx=14,pady=12); f.grid(row=2,column=0,sticky="nsew")
        top=tk.Frame(f,bg=CLR["panel"]); top.pack(fill="x",pady=(0,6))
        tk.Label(top,text="📋  Журнал",font=FONT_UI_B,fg=CLR["text"],bg=CLR["panel"]).pack(side="left")
        HoverButton(top,text="Очистить",command=self._clear_log,
                    bg=CLR["border"],bg_hover="#3A3D4A",fg=CLR["text_dim"]).pack(side="right")
        wr=tk.Frame(f,bg=CLR["border"],padx=1,pady=1); wr.pack(fill="both",expand=True)
        self._log=tk.Text(wr,bg=CLR["log_bg"],fg=CLR["log_info"],font=FONT_MONO,
        ...
        for tag,col in (("INFO",CLR["log_info"]),("DEBUG",CLR["log_debug"]),
                        ("ERROR",CLR["log_error"]),("WARNING",CLR["warning"]),("SUCCESS",CLR["log_ok"])):
    ...
    def _append(self,text,tag):
        self._log.config(state="normal"); self._log.insert("end",text+"\n",tag)
        self._log.see("end"); self._log.config(state="disabled")

    def _poll(self):
        try:
            while True:
                rec=self._q.get_nowait(); msg=rec.getMessage()
                tag=rec.levelname if rec.levelname in ("DEBUG","WARNING","ERROR") else "INFO"
                if any(w in msg for w in ("Готово","✔","сохранён","создан")): tag="SUCCESS"
                self._append(self._fmt.format(rec),tag)
        except queue.Empty: pass
        self.after(50,self._poll)

    def _upd_prog(self,pct,done,total):
        self._pb.set(pct); self._sz.config(text=f"{done:.1f} / {total:.1f} МБ")

    def _start(self):
        raw=self._pv.get().strip()
        if not raw: self._st.set("error","Не выбрана папка"); return
        regions=self._rp.get_selected()
        if not regions: self._st.set("error","Не выбран ни один регион"); return
        d=Path(raw)
        try: d.mkdir(parents=True,exist_ok=True)
        except Exception as ex: self._st.set("error",str(ex)); return
        cfg=dataclasses.replace(self._cfg,base_dir=d,region_folders=regions)
        self._lbl.config(text=f"Лог → {cfg.log_file}")
        self._stop.clear(); self._sbtn.set_enabled(False); self._cbtn.set_enabled(True)
        self._pb.set(0); self._sz.config(text=""); self._st.set("running")
        self._worker=threading.Thread(target=self._run,args=(cfg,),daemon=True); self._worker.start()

    def _cancel(self): self._stop.set(); self._cbtn.set_enabled(False)

    def _run(self,cfg):
        lg=setup_logger(cfg,self._q)
        lg.info("="*52); lg.info("Старт. Архив за: %s",cfg.last_tuesday)
        lg.info("Папка: %s",cfg.base_dir); lg.info("Регионов: %d",len(cfg.region_folders))
        lg.info("="*52)
        ...
        try:
            ...
            if self._stop.is_set(): lg.warning("Остановлено."); self.after(0,self._done_cancel); return
            purge_old(cfg,lg)
            lg.info("✔ Готово! Архив: %s",cfg.filtered_archive_path)
            self.after(0,self._done_ok)
        except InterruptedError: lg.warning("Остановлено."); self.after(0,self._done_cancel)
        except (RuntimeError,zipfile.BadZipFile,OSError) as ex:
            lg.error("Ошибка: %s",ex); self.after(0,self._done_err,str(ex))
        except Exception as ex:
            lg.exception("Непредвиденная ошибка: %s",ex); self.after(0,self._done_err,str(ex))

    def _done_ok(self): self._pb.set(100); self._st.set("done"); self._sbtn.set_enabled(True); self._cbtn.set_enabled(False)
    def _done_err(self,m): self._st.set("error",m[:70]); self._sbtn.set_enabled(True); self._cbtn.set_enabled(False)
    def _done_cancel(self): self._st.set("cancelled"); self._sbtn.set_enabled(True); self._cbtn.set_enabled(False)
